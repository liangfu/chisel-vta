...
class MemArbiter(implicit val p: Parameters) extends Module {
  ...
  io.out_cache.waitrequest := io.axi_master.waitrequest && (out_cache_write || out_cache_ack)
  ...
  io.ins_cache.waitrequest := (io.axi_master.waitrequest && ins_cache_read) || (!ins_cache_read && io.ins_cache.read)
  io.inp_cache.waitrequest := (io.axi_master.waitrequest && inp_cache_read) || (!inp_cache_read && io.inp_cache.read)
  io.wgt_cache.waitrequest := (io.axi_master.waitrequest && wgt_cache_read) || (!wgt_cache_read && io.wgt_cache.read)
  io.uop_cache.waitrequest := (io.axi_master.waitrequest && uop_cache_read) || (!uop_cache_read && io.uop_cache.read)
  io.acc_cache.waitrequest := (io.axi_master.waitrequest && acc_cache_read) || (!acc_cache_read && io.acc_cache.read)
